...
retex_defaults = OrderedDict(
    [
        ...
        ("aux_negative_prompt", "text, watermark, worst quality, low quality"),
        ("diff_bs", None),
        ("patch_size", 512),
        ("render_bs", 6),
        ("patch_bs", 1),
...
text_3d_to_3d_superres_params = dict(checkpoint="stabilityai/stable-diffusion-2-1-base")
...
def parse_3d_args(args, kwargs):
    ...
    superres_kwargs = {
        k: kwargs["superres_" + k] if "superres_" + k in kwargs else args.pop(0)
...
def parse_stablessdnerf_args(args, kwargs):
    stablessdnerf_kwargs = {
        k: kwargs[k] if k in kwargs else args.pop(0)
        for k in stablessdnerf_signatures.keys()
    }
