                ...
                with gr.Row():
                    for view_id in range(num_passes):
                        output_list.append(
                            gr.Image(
                                interactive=False,
                                label="Zero123++ output",
                                show_label=view_id == 0,
                                height=height,
                                min_width=140,
...
def create_interface_img_to_3d(
    ...
    default_var_dict = dict(
        init_inverse_steps=init_inverse_steps,
        n_inverse_steps=n_inverse_steps,
        diff_bs=diff_bs,
        tet_resolution=tet_resolution,
    )
    default_superres_var_dict = dict(n_inverse_steps=superres_n_inverse_steps)
    ...
    with gr.Blocks(analytics_enabled=False) as interface:
        with gr.Row():
            with gr.Column(scale=3):
                ...
                with gr.Accordion(
                    "Optional text prompts", open=True, elem_classes=["custom-spacing"]
                ) as prompt_accordion:
                ...
                if advanced:
                    ...
                    var_dict["checkpoint"] = gr.Textbox(
                        ...
                        lines=1,
                        value=nerf_mesh_defaults["checkpoint"],
                        elem_classes=["force-hide-container"],
                    )
                    create_auxiliary_prompt_opts(
                        var_dict,
                        **{
                            key: nerf_mesh_defaults[key]
                            for key in ["aux_prompt", "aux_negative_prompt"]
                        },
                    )
                    create_batch_size_opts(
                        var_dict,
                    ...
                    create_optimization_opts(
                        ...
                        **{
                            key: nerf_mesh_defaults[key]
                            for key in ["tet_init_inverse_steps", "start_lr", "end_lr"]
                        },
                    )
            ...
            with gr.Column(scale=2):
                ...
                create_generate_bar(
                    var_dict["gen_zero123plus"],
                    text="Run Zero123++ only",
                    variant="secondary",
                    seed=-1,
                )
                ...
                if pred_normal:
                    with gr.Tabs():
                        with gr.TabItem(f"RGB"):
                            create_passes(
                                var_dict["zero123plus_outputs"], num_passes, height=158
                            )
                        with gr.TabItem(f"Normal"):
                            create_passes(
                                var_dict["zero123plus_outputs"],
                                num_passes,
                                height=158,
                ...
                create_generate_bar(
                    var_dict["gen_mvedit"],
                    text="Run MVEdit only",
                    variant="secondary",
                    seed=-1,
                )
                var_dict["output"] = CustomModel3D(
                    height=400,
                    label="MVEdit output 3D model",
        ...
        default_var_dict = {
            k: default_var_dict.get(k, v)
            for k, v in nerf_mesh_defaults.items()
            if k not in var_dict
        }
        default_superres_var_dict = {
            "superres_" + k: default_superres_var_dict.get(k, v)
            for k, v in superres_defaults.items()
            if k not in var_dict["superres"]
        }
        img_to_3d_fun = partial(
            mvedit_api,
            **default_var_dict,
            **default_superres_var_dict,
        ...
        img_to_3d_inputs = (
            ...
            + [
                var_dict[k]
                for k in nerf_mesh_defaults.keys()
        ...
        var_dict["gen_all"]["run_btn"].click(
            ...
            outputs=var_dict["zero123plus_outputs"],
            concurrency_id="default_group",
